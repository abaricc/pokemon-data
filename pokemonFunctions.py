...
def readPokemonFromLine(line):
    #number, number hp, attack, defense, sp attack, sp defense, speed, total, average
    readList=line.split(" ")
    return Monster(name=readList[2], attack=int(readList[4]), defense=int(readList[5]), speed=int(readList[7]), hitpoint=[int(readList[3]),int(readList[3])], variety=[])
...
def readPokemonListFromFile(filename):
    ...
    with open(filepath) as file:
        for line in file:
            readList.append(readPokemonFromLine(line))
    file.close()
    return readList
...
def readPokemonTypesFromFileDictionary(filename):
    #returns a liat of Pokemons read from the file
    readDict={}
    THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
    filepath = os.path.join(THIS_FOLDER, filename)
    with open(filepath) as file:
        for line in file:
            name,typeList=readPokemonTypeFromLine(line)
            readDict[name]=typeList
    file.close()
    return readDict


#returns 0 if Pokemon1 wins, 1 if Pokemon2 wins and 2 if there is a draw
# Receives, the two pockemon fighting
# A boolean Value concerning whether or not the winner should replenish his HP after the match

def battle(Pokemon1, Pokemon2,resetPoints=False,verbose=False):
    ...
    if noEffect(Pokemon1, Pokemon2):
        ...
        attackCenter1=-1.5*effectiveness
    ...
    if noEffect(Pokemon2, Pokemon1):
        ...
        attackCenter2=-1.5*effectiveness
    ...
    if rounds==maxRounds:
        ...
        if (Pokemon1.hitpoint[1]>0):
            #Pokemons rest for the next battle and replenish HP if necessary
